...
export function serializeOrder(order: OrderDocument) {
  return {
    ...
    /* What the confirmation email quotes, so a customer writing in and the
       row in the admin list are talking about the same thing. The derived
       reference is the fallback for anything written before the sequence. */
    number: order.number ?? referenceNumber(order._id),
...
export function serializeSubscriptionPayment(payment: SubscriptionPaymentDocument) {
  return {
    ...
    number: payment.number ?? referenceNumber(payment._id),
